...
% Exercício 02: Programação Linear - SIMPLEX
%------------------------------------------------%
% 
...
% s.t. nb + .5*(na + nc) - p = 0
%      sqrt(3).5*(na - nc) - p = 0
%      -A*sig0 <= {na, nb, nc} <= A*sig0

% Adotar p = x1; na = x2; nb = x3; nc = x4; e A*sig0 = 1

% funcao f(x) = C'*x
C = [-1 0 0 0]'; % multiplicado por -1 para maximizar a funcao
...
% restricoes de igualdade do tipo A*x = b
A = [-2 1 2 1; -2 sqrt(3) 0 -sqrt(3)];
b = [0; 0];
fprintf('Restricoes:\n');
disp(table(A,b))
...
[xm, fm] = linprog(C,[],[],Aeq,beq,xlow,xupp);
