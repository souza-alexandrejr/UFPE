C*******************************************************************
C****                                                           ****
C**** SIMULATION OF TWO-PHASE OIL-WATER FLOW IN A HORIZONTAL    ****
C**** ONE-DIMENSIONAL SYSTEM USING THE IMPES FORMULATION AND    ****
C**** GAUSSIAN ELIMINATION                                      ****
C****                                                           ****
C**** MAXIMUM NUMBER OF GRID BLOCKS IS CURRENTLY SET AT 100     ****
C*******************************************************************
      PROGRAM OW
C
C-----DECLARATIONS
C
      ...
      REAL*8 LAMOM,LAMOP,LAMWM,LAMWP,DUMMY
...
C
      ...
      NULL=0
C
C----READ SYSTEM DATA FROM FILE SYST.DAT
C
      OPEN(12,FILE='SYST.DAT')
      READ(12,*)
C----USO AND USW ARE UPSTREAM WEIGHTING FACTORS FOR OIL AND WATER
C-------1.0 IS UPSTREAM
C-------0.0 OS DOWNSTREAM
C-------SHOULD BE A VALUE BETWEEN 0.0 AND 1.0 (STANDARD IS 1.0)
      READ(12,*)USO,USW
      READ(12,*)AREA
      READ(12,*)N
      READ(12,*)(DX(I),I=1,N)
      ...
      CLOSE(12)
C
C----GENERATE X-POSITIONS
C
      X(1)=DX(1)/2.
      DO 9 I=2,N
    9 X(I)=X(I-1)+(DX(I-1)+DX(I))/2.
C
C----DETERMINE WHICH LEFT SIDE BOUNDARY CONDIION TO BE USED
C----IF CONSTANT INJECTION RATE, IBC=2
C----IF CONSTANT LEFT SIDE PRESSURE, IBC=1
C
      IF(PL.NE.0.)THEN
      QWI=0.
      IBC=1
...
C
      PHEAD(N+3)="PR     "
C
C----READ TABLES FOR RELATIVE PERMEABILITIES AND CAPILLARY PRESSURES
C----FROM INPUT DATA FILE DATA FILE SAT.DAT
C
...
C----READ PVT DATA FROM INPUT DATA FILE PVT.DAT
C
...
C
C----CONVERT BO AND BW TO 1/BO OG 1/BW 
C
      BOT(J)=1./BOT(J)
      BWT(J)=1./BWT(J)
   20 CONTINUE
      CLOSE(11)
C
C-----OPEN FILES FOR PRINTOUT AND WRITE HEADINGS
C-----(PO.OUT FOR OIL PRESSURES, SW.OUT FOR WATER SATURATIONS
C-----AND WELLS.OUT FOR PRODUCTION/INJECTION RESULTS)
C
      OPEN(13,FILE='PO.OUT')
...
C
      DO 1000 J=1,1000
C
C----PRINTOUT OF TIME, PRESSURES, SATURATIONS AND PROD/INJ
C
...
C----------------------------INJECTION OF WATER AT LEFT SIDE 
C----------------------------REQUIRES SUM OF TRANSMISSIBILITIES
...
C
 ...
 1001 CONTINUE
      END
C
C----SUBROUTINE FOR INTERPOLATION OF DATA FROM INPUT TABLE
C
      SUBROUTINE INTERP(X,Y,DY,ISW,N,XT,YT)
C
C------------------------------------------------------------------
C     THE ROUTINE SEARCES IN THE TABLE AND CONDUCTS LINEAR INTERPO- 
C     LATION IN ORDER TO DETERMINE Y IN POINT X, AND ITS DERIVATIVE
C     IF ISW.NE.0
C     IF THE ARGUMENT (X) IS OUTSIDE THE TABLE, ENPOINTS ARE USED
C
C     X=ARGUMENT
C     Y=INTERPOLATED VALUE
C     DY=COMPUDED DERIVATIVE OF Y
C     ISW=0 IF DERIVATIVE IS NOT TO BE COMPUTED
C     N=NUMBER OF ENTRIES IN TABLE
C     XT=INDEPENDENT TABLE VARIABLE
C     YT=DEPENDENT TABLE VARIABLE
C------------------------------------------------------------------	 
C
      IMPLICIT REAL*8(A-H,O-Z)
      REAL*8 X,Y,DY,XT(100),YT(100)
C
C----IF X IS GREATER THAN LARGEST TABLE VALUE
C
      ...
      RETURN
C
C----IF X IS LESS THAN THE SMALLEST TABLE VALUE
C
